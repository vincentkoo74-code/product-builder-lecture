...
describe('Build17 QA persistence (Layer 1) — 실코드', () => {
  ...
  it('buildReport는 qa-report.v1 표준 스키마를 생성한다', () => {
    ...
    expect(r.build).toBe('26');
    expect(r.buildLabel).toBe('build26');
...
describe('Build17 QA file export (Layer 2) — 실코드', () => {
  it('Capacitor 없으면 클립보드 fallback으로 export한다', async () => {
    ...
    // 파일명: 금지문자(: 등) 없음, build26 스탬프.
    expect(out.filename).toMatch(/^qa-report-build26-\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}\.json$/);
  ...
  it('Capacitor Filesystem이 있으면 Documents에 저장하고 Share한다', async () => {
    ...
    expect(calls.write.path).toMatch(/^qa-report-build26-.*\.json$/);
    ...
    expect(calls.share.title).toBe('QA Report build26');
  });
});
