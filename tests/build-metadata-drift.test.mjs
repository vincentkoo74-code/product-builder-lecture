...
describe('빌드 메타데이터 — 진실 소스', () => {
  ...
  it('[계약 1] 이번 QA 릴리즈의 Apple 바이너리 build number는 49이다 (제품=Build47-Codex — 47/48은 ASC 소진)', () => {
    expect(CANONICAL_BUILD).toBe(49);
  });
});
...
describe('빌드 메타데이터 — index.html이 진실 소스를 미러한다', () => {
  ...
  it('[계약 4] buildLabel은 하드코딩이 아니라 BUILD_NUMBER에서 파생된다', () => {
    ...
    expect(html).toContain("const PRODUCT_BUILD_LABEL = '47-codex';");
...
describe('빌드 메타데이터 — 리포트 실제 산출값', () => {
  ...
  const PRODUCT_BUILD_LABEL = '47-codex';
  const QA_BUILD_LABEL = 'build' + PRODUCT_BUILD_LABEL;

  it('[계약 3] report.build == canonical(바이너리 49)의 문자열 표현', () => {
    expect(String(BUILD_NUMBER)).toBe('49');
  });

  it('[계약 4] report.buildLabel == "build47-codex" (제품 라벨)', () => {
    expect(QA_BUILD_LABEL).toBe('build47-codex');
  });

  it('[계약 5-CODEX] 제품 라벨의 기본 숫자(47)+2 == 바이너리 CPV(49) — ASC 소진 오프셋 명시', () => {
    const base = Number((QA_BUILD_LABEL.match(/^build(\d+)-codex$/) || [])[1]);
    expect(base + 2).toBe(CANONICAL_BUILD);
  });
});
