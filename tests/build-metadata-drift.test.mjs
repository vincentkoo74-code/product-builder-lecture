...
describe('빌드 메타데이터 — 진실 소스', () => {
  ...
  it('[계약 1] 이번 릴리즈의 build number는 40이다', () => {
    expect(CANONICAL_BUILD).toBe(40);
  });
});
...
describe('빌드 메타데이터 — 리포트 실제 산출값', () => {
  ...
  it('[계약 3] report.build == canonical의 문자열 표현', () => {
    expect(String(BUILD_NUMBER)).toBe('40');
  });

  it('[계약 4] report.buildLabel == "build40"', () => {
    expect(QA_BUILD_LABEL).toBe('build40');
  });
