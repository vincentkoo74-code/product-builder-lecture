...
--       라이브 대조로 재확인했다.
--
-- ⚠️ 파일명이 2026-01-01 인 이유 — 스테이징 검증에서 발견된 실제 결함 때문이다.
--   처음에는 이 파일을 20260827001000 으로 두었다. 그 결과 빈 DB 에 적용할 때
--   20260806013625(participants 에 컬럼 추가)가 **이 baseline 보다 먼저** 실행되어
--       ERROR: relation "public.participants" does not exist (SQLSTATE 42P01)
--   로 clean bootstrap 이 실패했다. 즉 "저장소만으로 재현 가능"이 성립하지 않았다.
--   baseline 은 정의상 **모든 증분보다 먼저** 와야 하므로 전체 세트의 최선두로 재배치했다.
--   과거 날짜는 "역사적 마이그레이션인 척"이 아니라 **정렬 순서를 위한 baseline 표식**이다.
--   라이브 Tokyo 의 원장에는 이 버전이 없으므로 배포 시 `--include-all` 이 필요하다(설계 문서 §5).
--
