...
| DR-6 | 음성 단일 소스 정책(녹음/TTS 중 하나, 로케일 분리). MP3 매핑 이벤트는 TTS 금지.<br>**예외(Build19, WRPS-052-B19)**: `intro`(ko) 1개 이벤트에 한해 TTS_OVERRIDE로 mp3보다 우선. **최초 근거는 부정확했음** — "mp3에 이미 풀구호가 녹음돼 있다"는 06-26/28 청취 기록을 신뢰해 반대했으나, 2026-07-08 whisper(OpenAI) 전사로 `ASSETS/rps/voice/ko/` 14개 파일 전체를 실제 확인한 결과 **"안 내면 술래 가위바위보!!" 문구를 담은 파일이 애초에 존재하지 않음**이 확정됨(`ko_game_start.mp3`="게임을 시작합니다." — 전혀 다른 문구). 즉 TTS는 "정확한 mp3 대신 도입한 위험"이 아니라 "존재하지 않는 사람 목소리를 대신하는 유일한 정확 수단". 신규 MC 녹음 준비될 때까지 유지 결정(사용자, 2026-07-08). **DR-10 원칙대로 실기기 가청 확인 전까지 Evidence-gated**(닫지 않음) — TTS `onend`는 utterance 완주만 보장, 실제 가청 여부는 보장 못 함(WKWebView 오디오 세션 충돌 등으로 무음 가능). | WRPS-045, WRPS-052-B19 |
